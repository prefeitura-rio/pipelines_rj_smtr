    ...
    with Flow(flow_name) as default_materialization_flow:
        ...
        if pre_test:
            dbt_pre_test = run_dbt(
                resource="test",
                test_name=pre_test["test_name"] if pre_test else None,
                dataset_id=pre_test["dataset_id"] if pre_test else None,
                table_id=pre_test["table_id"] if pre_test else None,
                model=pre_test["model"] if pre_test else None,
                flags=flags,
                _vars=dbt_run_vars,
            ...
            notify_pre_test = dbt_data_quality_checks(
                ...
                checks_list=pre_test["checks_list"] if pre_test else None,
                params=dbt_run_vars,
            )
        ...
        if post_test:
            dbt_post_test = run_dbt(
                resource="test",
                test_name=post_test["test_name"] if post_test else None,
                dataset_id=post_test["dataset_id"] if post_test else None,
                table_id=post_test["table_id"] if post_test else None,
                model=post_test["model"] if post_test else None,
                flags=flags,
                _vars=dbt_run_vars,
            ...
            notify_post_test = dbt_data_quality_checks(
                ...
                checks_list=post_test["checks_list"] if post_test else None,
                params=dbt_run_vars,
            )
