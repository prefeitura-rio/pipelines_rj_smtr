...
def create_api_url_onibus_realocacao(
    ...
    secret_path: str = constants.GPS_SPPO_API_SECRET_PATH.value,
) -> str:
    """
    ...
    url = "https://integration.systemsatx.com.br/Globalbus/SMTR/EnvioViagensRetroativas?"
...
def pre_treatment_br_rj_riodejaneiro_onibus_realocacao(status: dict, timestamp: datetime) -> Dict:
    ...
    for col in dt_cols:
        ...
        ).dt.tz_localize(tz=emd_constants.TIMEZONE.value)
...
def pre_treatment_br_rj_riodejaneiro_onibus_gps(
    ...
    error = None
    timezone = emd_constants.TIMEZONE.value
