...
{%- if execute %}
    {% set query = (
        ...
        ~ "rj-smtr-dev.janaina__revisaoLECD__projeto_subsidio_sppo.subsidio_data_versao_efetiva"
...
with
    ...
    data_versao_efetiva as (
        ...
        from rj-smtr-dev.janaina__revisaoLECD__projeto_subsidio_sppo.subsidio_data_versao_efetiva
    ...
    servico_faixa_km_apuracao as (
        select
            ...
            pof
        from `rj-smtr-dev.janaina__revisaoLECD__subsidio.percentual_operacao_faixa_horaria`
        where {{ incremental_filter }}
    ),
    viagem_km_tipo as (
        select distinct
