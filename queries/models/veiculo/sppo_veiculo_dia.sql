...
with
    licenciamento_data_versao as (
        select * from {{ ref("licenciamento_data_versao_efetiva") }}
    ),
    infracao_data_versao as (select * from {{ ref("infracao_data_versao_efetiva") }}),
    licenciamento as (
        select
        ...
        {% if is_incremental() %}
            where dve.data = date("{{ var('run_date') }}")
        {% endif %}
    ),
    gps as (
        ...
        where
            {% if is_incremental() %} data = date("{{ var('run_date') }}")
    ...
    autuacoes as (
        ...
        {% if is_incremental() %}
            where data_infracao = date("{{ var('run_date') }}")
        {% endif %}
    ),
