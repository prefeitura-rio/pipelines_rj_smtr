...
with
    tipo_deficiencia as (
        select
            cast(d.id_laudo_pcd as int64) as id_laudo_pcd,
            d.id_tipo_doenca,
            d.tipo_doenca as tipo_deficiencia,
            d.data_inclusao as datetime_inclusao,
            --datetime(timestamp(d.data_inclusao) - interval 3 hour) as datetime_captura,
            p.timestamp_captura as datetime_captura
        from `rj-smtr-dev.adriano__bilhetagem.tipo_deficiencia` d
        join
            {{ ref("staging_laudo_pcd") }} p
            on d.id_laudo_pcd = p.cast(cd_cliente as int64)
    ...
    dados_completos_deduplicados as (
        ...
        qualify
            row_number() over (
                partition by id_laudo_pcd, datetime_inclusao
        ...
        select
            *,
            lag(datetime_inclusao) over (
                partition by id_laudo_pcd order by datetime_inclusao
        ...
        select
            *,
            lead(datetime_inicio_validade) over (
                partition by id_laudo_pcd order by datetime_inicio_validade
            ) as datetime_fim_validade
        from inicio_validade
    )
select
    concat(id_laudo_pcd, '-', datetime_inclusao) as id_unico,
    id_laudo_pcd,
    row_number() over (
        partition by id_laudo_pcd order by datetime_inicio_validade
    ) as sequencia_laudo_pcd,
    id_tipo_doenca,
    tipo_deficiencia,
