    ...
    aux_status as (
        select
            *,
            ...
            = 'middleend' as ends
        from {{ ref("aux_registros_status_trajeto") }}
        {% if var("tipo_materializacao") == "monitoramento" %}
            where indicador_segmento = true
        {% endif %}
    ),
...
select distinct
    ...
    sentido_shape,
    round(
        (st_distance(start_pt, posicao_partida) + st_distance(end_pt, posicao_chegada))
        / 1000,
        3
    ) as distancia_inicio_fim,
    distancia_planejada,
    shape_id_planejado,
    trip_id_planejado,
    sentido,
