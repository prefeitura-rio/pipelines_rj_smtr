...
def model(dbt, session):
    ...
    dbt.config(partition_by={"field": "feed_start_date", "data_type": "date", "granularity": "day"})
    ...
    shapes_df["ponto_shape"] = shapes_df[["shape_pt_lon", "shape_pt_lat"]].apply(
        lambda row: shapely.Point(row["shape_pt_lon"], row["shape_pt_lat"]), axis=1
    )
    ...
    # PTS
    contents["final_pt_sequence"] = contents.groupby(["feed_start_date", "shape_id"])[
        "shape_pt_sequence"
    ].transform("max")
    ...
    # Shapes
    shapes = (
        pts.groupby(["shape_id", "feed_start_date"])
        .agg(
            {
    ...
    half_0 = pts.groupby(["shape_id", "feed_start_date"]).apply(
        lambda df: create_half_shapes(df, df["final_pt_sequence"].iloc[0] // 2)
    )
    half_1 = pts.groupby(["shape_id", "feed_start_date"]).apply(
        lambda df: create_half_shapes(df, df["final_pt_sequence"].iloc[0] // 2 + 1)
    )
    ...
    shapes_half_0 = (
        half_0.groupby(["shape_id", "feed_start_date", "new_shape_id"])
        .agg(
            {
                "ponto_shape": lambda x: shapely.LineString(x.tolist()),
                "ponto_shape": ["first", "last"],
    ...
    shapes_half_1 = (
        half_1.groupby(["shape_id", "feed_start_date", "new_shape_id"])
        .agg(
            {
                "ponto_shape": lambda x: shapely.LineString(x.tolist()),
                "ponto_shape": ["first", "last"],
    ...
    union_shapes = union_shapes[
        (round(union_shapes["start_pt"].y, 4) == round(union_shapes["end_pt"].y, 4))
