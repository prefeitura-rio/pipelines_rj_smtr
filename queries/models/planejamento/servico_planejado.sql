...
{# {% set ordem_servico_trajeto_alternativo = ref("ordem_servico_trajeto_alternativo_gtfs") %} #}
{% set ordem_servico_trajeto_alternativo = (
...
with
    ...
    trip_info as (
        ...
        left join
            {# {{ ref("shapes_geom_gtfs") }} s  #}
            `rj-smtr.gtfs.shapes_geom` s using (feed_start_date, feed_version, shape_id)
        qualify
            row_number() over (
                partition by
                    ta.feed_start_date,
